...
def test_init_with_yes_installs_recommended_capabilities(tmp_path, monkeypatch, fake_precommit):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "Dockerfile").write_text("")
...
def test_init_always_offers_secrets_even_with_no_detection(tmp_path, monkeypatch, fake_precommit):
    monkeypatch.chdir(tmp_path)
...
def test_init_persists_config_even_if_hook_install_fails(tmp_path, monkeypatch, fake_precommit):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(precommit, "install_hooks", lambda config=None: False)
